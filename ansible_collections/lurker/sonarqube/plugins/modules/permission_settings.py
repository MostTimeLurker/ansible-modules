...
def run_module():
    module_args = dict(
        ...
        permissions=dict(type='list', required=False, default=["user"], choices=["admin", "profileadmin", "gateadmin", "scan", "provisioning"]),
...
# if state == absent -> if permission set -> remove permission, else remove group
    ...
    try:
        if (state == "present"):
           ...
           sonar_result = sonar.permissions.add_permission_to_group(groupName=group, permission=permissions)
        elif (state == "absent"):
            sonar_result = sonar.permissions.remove_permission_from_group(groupName=group, permission=permissions)
        else:

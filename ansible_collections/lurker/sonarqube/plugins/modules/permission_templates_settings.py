...
DOCUMENTATION = r"""
---
module: permission_templates_settings.py
"""
...
def run_module():
    module_args = dict(
        ...
        permissions=dict(type='list', required=False, default=["user"], choices=["admin", "codeviewer", "issueadmin", "securityhotspotadmin", "scan", "user"]),
        state=dict(type='str', default='present', choices=['present', 'absent'])
    )

# if state == present -> permissions are required
# if state == absent -> if permission set -> remove permission, else remove group
    ...
    try:
        if (state == "present"):
           ## if there are permissions set by gui, they wont get removed/logged by this script
           sonar_result = sonar.permissions.add_group_to_template(groupName=group, templateName=name, permission=permissions)
        elif (state == "absent"):
            sonar_result = sonar.permissions.remove_group_from_template(groupName=group, templateName=name, permission=permissions)
        else:
